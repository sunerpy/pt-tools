...
- **精确定时**：为每个种子创建独立定时器，在免费结束时刻精确触发检查
- **智能暂停**：免费期结束时自动暂停未完成的下载任务
- **完成检测**：已完成的种子不会被暂停，继续正常做种
- **手动恢复**：支持在 Web 界面手动恢复暂停的任务
- **批量管理**：支持批量删除暂停任务（可选是否同时删除数据文件）
- **历史归档**：查看历史暂停记录和处理结果
- **重试机制**：暂停操作失败时自动重试（最多 3 次）
- **周期巡检**：每 5 分钟兜底检查，确保不遗漏任何任务
...
此功能用于管理 RSS 订阅中下载的免费种子，当免费期结束时自动暂停未完成的下载任务，避免产生额外的下载量。
...
1. 进入「暂停任务管理」页面（侧边栏菜单）
2. 查看所有被系统暂停的种子列表
3. 可执行以下操作：

| 操作               | 说明                                   |
| ------------------ | -------------------------------------- |
| **恢复下载**       | 恢复单个任务继续下载（不再受免费限制） |
| **删除任务**       | 从下载器删除任务，保留数据文件         |
| **删除任务和数据** | 同时删除任务和已下载的数据文件         |
| **批量删除**       | 选中多个任务后批量删除                 |
...
1. **仅对免费种子生效**：非免费种子不会被监控
2. **需要下载器支持**：确保下载器配置正确且连接正常
3. **时间精度**：系统采用双重机制确保及时检查
    - **精确定时器**：为每个种子创建独立定时器，在免费结束时刻精确触发
    - **周期性巡检**：每 2 分钟检查，处理可能遗漏的任务
4. **重试机制**：暂停操作失败会自动重试最多 3 次
5. **手动恢复后**：恢复的任务不再受免费限制监控
...
| 下载路径           | 指定下载目录（可选）                         |
| 过滤规则           | 关联的过滤规则（可选）                       |
| **免费结束时暂停** | 开启后，免费期结束时自动暂停未完成的下载任务 |
...
1. 确认 RSS 订阅已开启「免费结束时暂停」选项
2. 检查种子是否为免费种子（非免费种子不会被监控）
3. 确认下载器配置正确且连接正常
4. 查看日志确认监控器是否正常运行
5. 检查种子是否有免费结束时间信息
