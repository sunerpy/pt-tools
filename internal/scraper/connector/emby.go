...
// EmbyConnector implements core.MediaServerConnector for Emby servers.
// It reuses baseClient, which also supports Jellyfin (APIs are ~95% compatible).
type EmbyConnector struct {
	base baseClient
}

// NewEmbyConnector constructs an Emby connector.
// baseURL: server address (with scheme, e.g., "http://localhost:6789")
// apiKey: X-Emby-Token header value
// client: *http.Client to use (nil defaults to http.DefaultClient)
func NewEmbyConnector(baseURL, apiKey string, client *http.Client) *EmbyConnector {
	if client == nil {
...
func (c *EmbyConnector) Name() string {
	return "emby"
}
...
func (c *EmbyConnector) Ping(ctx context.Context) (*core.ServerInfo, error) {
	var info PublicInfo
...
func (c *EmbyConnector) Authenticate(ctx context.Context) (*core.ServerInfo, error) {
	var info PublicInfo
...
func (c *EmbyConnector) ListLibraries(ctx context.Context) ([]core.Library, error) {
...
func (c *EmbyConnector) RefreshLibrary(ctx context.Context, libraryID string) error {
...
func (c *EmbyConnector) ScanStatus(ctx context.Context) (*core.ScanStatus, error) {
