...
var HDTimeDefinition = &v2.SiteDefinition{
	ID:             "hdtime",
	Name:           "HDTime",
	Aka:            []string{"HDTIME"},
	Description:    "HDTime 综合影视 PT 站点",
	Schema:         v2.SchemaNexusPHP,
	URLs:           []string{"https://hdtime.org/"},
	FaviconURL:     "https://hdtime.org/favicon.ico",
		...
		Selectors: map[string]v2.FieldSelector{
			"id": {
				Selector: []string{
					...
					"span.nowrap a[href*='userdetails.php']",
			...
			"uploaded": {
				Selector: []string{"td.rowhead:contains('传输') + td", "td.rowhead:contains('Transfer') + td", ".bottom .medium", "body"},
				Attr:     "html",
				Filters: []v2.Filter{
			...
			"ratio": {
				Selector: []string{"td.rowhead:contains('传输') + td", "td.rowhead:contains('Ratio') + td", ".bottom .medium", "body"},
				Attr:     "html",
				Filters: []v2.Filter{
...
func init() {
	v2.RegisterSiteDefinition(HDTimeDefinition)
}
