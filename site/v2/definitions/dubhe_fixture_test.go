...
func init() {
	RegisterFixtureSuite(FixtureSuite{SiteID: "dubhe", Search: testDubheSearch, Detail: testDubheDetail, UserInfo: testDubheUserInfo})
}

const dubheSearchFixture = `<html><body>
...
<tr>
  ...
  <td class="rowfollow"><table class="torrentname"><tr><td class="embedded">
    <a href="details.php?id=38647">Dubhe.Demo.2026-CarPT</a>
...
<tr>
  ...
  <td class="rowfollow"><table class="torrentname"><tr><td class="embedded">
    <a href="details.php?id=386477">Dubhe.Demo2.FLAC-CarPT</a>
...
const dubheIndexFixture = `<html><body>
<table id="info_block"><tr><td>
欢迎回来, <span class="nowrap"><a href="https://carpt.net/userdetails.php?id=10821" class='User_Name'><b>dubhe_user</b></a></span>
...
const dubheUserdetailsFixture = `<html><body>
<table>
...
const dubheDetailFixture = `<html><body>
<h1 align="center" id="top" value="Dubhe.Demo.2026-CarPT"><input name="torrent_name" type="hidden" value="Dubhe.Demo.2026-CarPT" /><input name="detail_torrent_id" type="hidden" value="38647" />Dubhe.Demo.2026-CarPT <b>[<font class='free'>免费</font>]</b> <font color='#0000FF'>剩余时间：<span title="2026-04-04 13:01:22">3天21时</span></font><img class="hitandrun" src="pic/trans.gif" alt="H&amp;R" title="H&amp;R" /></h1>
<table>
...
func getDubheDef(t *testing.T) *v2.SiteDefinition {
	t.Helper()
	def, ok := v2.GetDefinitionRegistry().Get("dubhe")
	require.True(t, ok)
	return def
}

func testDubheSearch(t *testing.T) {
	def := getDubheDef(t)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(dubheSearchFixture))
	}))
	defer server.Close()
	...
	assert.Equal(t, "38647", items[0].ID)
	assert.Equal(t, "Dubhe.Demo.2026-CarPT", items[0].Title)
	...
	assert.Equal(t, "386477", items[1].ID)
	assert.Equal(t, v2.DiscountPercent50, items[1].DiscountLevel)
}

func testDubheDetail(t *testing.T) {
	def := getDubheDef(t)
	parser := v2.NewNexusPHPParserFromDefinition(def)

	doc := FixtureDoc(t, "dubhe_detail", dubheDetailFixture)
	info := parser.ParseAll(doc.Selection)
	assert.Equal(t, "38647", info.TorrentID)
	assert.Equal(t, "Dubhe.Demo.2026-CarPT", info.Title)
	assert.Equal(t, v2.DiscountFree, info.DiscountLevel)
	assert.True(t, info.HasHR)
	assert.InDelta(t, 4096, info.SizeMB, 0.1)
}

func testDubheUserInfo(t *testing.T) {
	def := getDubheDef(t)
	driver := newTestNexusPHPDriver(def)

	indexDoc := FixtureDoc(t, "dubhe_index", dubheIndexFixture)
	assert.Equal(t, "10821", driver.ExtractFieldValuePublic(indexDoc, def.UserInfo.Selectors["id"]))
	assert.Equal(t, "dubhe_user", driver.ExtractFieldValuePublic(indexDoc, def.UserInfo.Selectors["name"]))
	...
	userDoc := FixtureDoc(t, "dubhe_userdetails", dubheUserdetailsFixture)
...
func TestDubhe_Fixtures_NoSecrets(t *testing.T) {
	fixtures := map[string]string{
		"search":      dubheSearchFixture,
		"index":       dubheIndexFixture,
		"userdetails": dubheUserdetailsFixture,
		"detail":      dubheDetailFixture,
	}
