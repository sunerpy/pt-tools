...
var HXPTDefinition = &v2.SiteDefinition{
	ID:             "hxpt",
	Name:           "HXPT",
	Aka:            []string{"好学"},
	Description:    "以教育类资源为主的 NexusPHP PT 站点",
	Schema:         v2.SchemaNexusPHP,
	URLs:           []string{"https://www.hxpt.org/"},
	FaviconURL:     "https://www.hxpt.org/favicon.ico",
	TimezoneOffset: "+0800",
	RateLimit:      0.5,
	RateBurst:      2,
	UserInfo: &v2.UserInfoConfig{
		...
		Selectors: map[string]v2.FieldSelector{
			...
			"bonus": {
				Selector: []string{"#info_block", "td.rowhead:contains('火花') + td", "td.rowhead:contains('Bonus') + td"},
				Attr:     "html",
				Filters:  []v2.Filter{{Name: "regex", Args: []any{`(?:火花|Bonus)[^\d]*([\d.,]+)`}}, {Name: "parseNumber"}},
			},
			"joinTime": {
	...
	Selectors: &v2.SiteSelectors{
		...
		Subtitle:           "table.torrentname div.torrent-subtitle, table.torrentname td.embedded > span:last-of-type, table.torrentname td.embedded > span:not(.optiontag)",
		...
		DiscountIcon:       "img.pro_free, img.pro_free2up, img.pro_50pctdown, img.pro_30pctdown, img.pro_2up, img.pro_50pctdown2up, font.free",
...
func init() {
	v2.RegisterSiteDefinition(HXPTDefinition)
}
