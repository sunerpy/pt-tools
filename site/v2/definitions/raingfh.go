...
var RaingfhDefinition = &v2.SiteDefinition{
	ID:             "raingfh",
	Name:           "RainGFH",
	Aka:            []string{"雨", "raingfh.top"},
	Description:    "雨 PT 站",
	Schema:         v2.SchemaNexusPHP,
	URLs:           []string{"https://raingfh.top/"},
	FaviconURL:     "https://raingfh.top/favicon.ico",
	...
	UserInfo: &v2.UserInfoConfig{
		...
		Selectors: map[string]v2.FieldSelector{
			...
			"name": {
				Selector: []string{"a[href*='userdetails.php'][class*='Name']", "#info_block a[href*='userdetails.php']", "h1 .nowrap b", "h1 b"},
			},
			"uploaded": {
			...
			"ratio": {
				...
				Attr:     "html",
				Filters: []v2.Filter{{Name: "regex", Args: []any{`(?:分享率|Ratio)[^\d∞I-]*(?:</font>)?\s*([\d.,]+|∞|Inf)`}}, {
					Name: "parseNumber",
				}},
			},
			"levelName": {
				Selector: []string{"td.rowhead:contains('等级') + td img"},
				Attr:     "title",
			},
			"bonus": {
				Selector: []string{"#info_block", "td.rowhead:contains('雨滴') + td"},
				Attr:     "html",
				Filters:  []v2.Filter{{Name: "regex", Args: []any{`雨滴\s*</font>\s*\[[^\]]*\]\s*[:：]\s*([\d.,]+)`}}, {Name: "parseNumber"}},
			},
			"joinTime": {
				Selector: []string{"td.rowhead:contains('加入日期') + td"},
	...
	Selectors: &v2.SiteSelectors{
		...
		Category:           "td.rowfollow:nth-child(1) img[alt], td.rowfollow:nth-child(1) img[title]",
...
func init() {
	v2.RegisterSiteDefinition(RaingfhDefinition)
}
