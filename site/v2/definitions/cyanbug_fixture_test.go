...
func init() {
	RegisterFixtureSuite(FixtureSuite{SiteID: "cyanbug", Search: testCyanBugSearch, Detail: testCyanBugDetail, UserInfo: testCyanBugUserInfo})
}

const cyanbugSearchFixture = `<html><body>
...
<tr>
  ...
  <td class="rowfollow"><table class="torrentname"><tr><td class="embedded">
    <a href="details.php?id=20077">CyanBug.Demo.2026-CarPT</a>
...
<tr>
  ...
  <td class="rowfollow"><table class="torrentname"><tr><td class="embedded">
    <a href="details.php?id=200777">CyanBug.Demo2.FLAC-CarPT</a>
...
const cyanbugIndexFixture = `<html><body>
<table id="info_block"><tr><td>
欢迎回来, <span class="nowrap"><a href="https://carpt.net/userdetails.php?id=19953" class='User_Name'><b>cyanbug_user</b></a></span>
...
const cyanbugUserdetailsFixture = `<html><body>
<table>
...
const cyanbugDetailFixture = `<html><body>
<h1 align="center" id="top" value="CyanBug.Demo.2026-CarPT"><input name="torrent_name" type="hidden" value="CyanBug.Demo.2026-CarPT" /><input name="detail_torrent_id" type="hidden" value="20077" />CyanBug.Demo.2026-CarPT <b>[<font class='free'>免费</font>]</b> <font color='#0000FF'>剩余时间：<span title="2026-04-04 13:01:22">3天21时</span></font><img class="hitandrun" src="pic/trans.gif" alt="H&amp;R" title="H&amp;R" /></h1>
<table>
...
func getCyanBugDef(t *testing.T) *v2.SiteDefinition {
	t.Helper()
	def, ok := v2.GetDefinitionRegistry().Get("cyanbug")
	require.True(t, ok)
	return def
}

func testCyanBugSearch(t *testing.T) {
	def := getCyanBugDef(t)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(cyanbugSearchFixture))
	}))
	defer server.Close()
	...
	assert.Equal(t, "20077", items[0].ID)
	assert.Equal(t, "CyanBug.Demo.2026-CarPT", items[0].Title)
	...
	assert.Equal(t, "200777", items[1].ID)
	assert.Equal(t, v2.DiscountPercent50, items[1].DiscountLevel)
}

func testCyanBugDetail(t *testing.T) {
	def := getCyanBugDef(t)
	parser := v2.NewNexusPHPParserFromDefinition(def)

	doc := FixtureDoc(t, "cyanbug_detail", cyanbugDetailFixture)
	info := parser.ParseAll(doc.Selection)
	assert.Equal(t, "20077", info.TorrentID)
	assert.Equal(t, "CyanBug.Demo.2026-CarPT", info.Title)
	assert.Equal(t, v2.DiscountFree, info.DiscountLevel)
	assert.True(t, info.HasHR)
	assert.InDelta(t, 4096, info.SizeMB, 0.1)
}

func testCyanBugUserInfo(t *testing.T) {
	def := getCyanBugDef(t)
	driver := newTestNexusPHPDriver(def)

	indexDoc := FixtureDoc(t, "cyanbug_index", cyanbugIndexFixture)
	assert.Equal(t, "19953", driver.ExtractFieldValuePublic(indexDoc, def.UserInfo.Selectors["id"]))
	assert.Equal(t, "cyanbug_user", driver.ExtractFieldValuePublic(indexDoc, def.UserInfo.Selectors["name"]))
	...
	userDoc := FixtureDoc(t, "cyanbug_userdetails", cyanbugUserdetailsFixture)
...
func TestCyanBug_Fixtures_NoSecrets(t *testing.T) {
	fixtures := map[string]string{
		"search":      cyanbugSearchFixture,
		"index":       cyanbugIndexFixture,
		"userdetails": cyanbugUserdetailsFixture,
		"detail":      cyanbugDetailFixture,
	}
