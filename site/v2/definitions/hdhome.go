...
var HDHomeDefinition = &v2.SiteDefinition{
	ID:             "hdhome",
	Name:           "HDHome",
	Aka:            []string{"HDHome", "家园"},
	Description:    "高清影视 PT 站点",
	Schema:         v2.SchemaNexusPHP,
	URLs:           []string{"https://hdhome.org/"},
	FaviconURL:     "https://hdhome.org/favicon.ico",
	TimezoneOffset: "+0800",
	RateLimit:      0.5,
	RateBurst:      2,
	UserInfo: &v2.UserInfoConfig{
		...
		Selectors: map[string]v2.FieldSelector{
			...
			"uploaded": {
				Selector: []string{"#info_block", "td.rowhead:contains('上传量') + td", "td.rowhead:contains('上傳量') + td", "td.rowhead:contains('Uploaded') + td", "td.rowhead:contains('传输') + td", "td.rowhead:contains('傳輸') + td"},
				Attr:     "html",
				Filters: []v2.Filter{
			...
			"downloaded": {
				Selector: []string{"#info_block", "td.rowhead:contains('下载量') + td", "td.rowhead:contains('下載量') + td", "td.rowhead:contains('Downloaded') + td", "td.rowhead:contains('传输') + td", "td.rowhead:contains('傳輸') + td"},
				Attr:     "html",
				Filters: []v2.Filter{
			...
			"ratio": {
				Selector: []string{"#info_block", "td.rowhead:contains('分享率') + td font", "td.rowhead:contains('分享率') + td", "td.rowhead:contains('Ratio') + td font", "td.rowhead:contains('Ratio') + td", "td.rowhead:contains('传输') + td", "td.rowhead:contains('傳輸') + td"},
				Attr:     "html",
				Filters: []v2.Filter{
					{Name: "regex", Args: []any{`(?:分享率|分享率：|Ratio)[^\d∞]*([\d.,]+|∞|Inf)`}},
					{Name: "parseNumber"},
				},
			},
			"levelName": {
	...
	Selectors: &v2.SiteSelectors{
		...
		Subtitle:           "table.torrentname td.embedded > span[style*='float:left']",
...
func init() {
	v2.RegisterSiteDefinition(HDHomeDefinition)
}
