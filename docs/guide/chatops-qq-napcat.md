...
![NapCat WebUI 登录页](images/chatops/napcat-webui-login.png)

> **截图位置**：浏览器访问 `http://<host>:6099/webui`，出现 token 输入框的页面。请自行截图。
...
![NapCat 扫码登录](images/chatops/napcat-qq-login.png)

> **截图位置**：NapCat WebUI → 登录 → 二维码页面。请自行截图。

![NapCat 登录成功状态](images/chatops/napcat-qq-online.png)

> **截图位置**：登录成功后 WebUI 首页，显示「已登录」和 QQ 号。请自行截图。
...
![NapCat 网络配置页](images/chatops/napcat-network-config.png)

> **截图位置**：NapCat WebUI → 网络配置 → WebSocket 客户端，填写完毕后的状态。请自行截图。
