...
| --- | --- | --- |
| **不通知** | - | 关闭此 RSS 的通知 |
| **每条新种（不拉详情）** | RSS feed 拉到一条新条目时立即触发 | 任何新种都通知我，仅用 feed 标题/链接，不消耗站点请求 |
| **仅匹配过滤规则的种子** | 详情页拉完、过滤规则命中时触发 | 只通知"我真的要追的剧 / 蓝光原盘"，需要详情才能判断 |
| **以上两种都开** | 上面两路都启用（合并展示） | 既不漏种，又能在命中规则时高亮，避免重复通知 |
...
- **通知模式**：选"每条新种（不拉详情）" / "仅匹配过滤规则的种子" / "以上两种都开"，留空表示关闭
...
`both` 启用时同一条种子可能产生 **两条** 候选（"每条新种" + "仅匹配规则"），但 pt-tools 会自动避免重复打扰：

- 当"仅匹配规则"触发时，如果同一 `(rss_id, site_name, torrent_id)` 已有"每条新种"行处于 `pending` 状态（还没真发出去），自动改为 `suppressed`，只发"仅匹配规则"这一条；
- 如果"每条新种"行已经 `sent`（已经飞出去了），不会撤回，但"仅匹配规则"仍然正常推送。
