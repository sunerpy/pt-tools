...
| 路径 | 触发时机 | 用途 |
| --- | --- | --- |
| **all** | RSS feed 拉到一条新条目时立即触发 | 任何新种都告诉我（不一定要下载） |
| **filtered** | 详情页拉完、过滤规则命中时触发 | 只通知"我真的要追的剧 / 蓝光原盘" |
| **both** | 上面两路都启用 | 既不漏，又能在命中规则时高亮 |
...
- **通知模式**：选 `all` / `filtered` / `both`，留空表示关闭
...
## 两种通知模式：all vs filtered

### all 模式

- **触发时机**：RSS feed 拿到 `<item>` 那一瞬间
...
### both 模式：自动 supersede

`both` 启用时同一条种子可能产生 **两条** 候选（all + filtered），但 pt-tools 会自动避免重复打扰：

- 当 filtered 触发时，如果同一 `(rss_id, site_name, torrent_id)` 已有 `all` 行处于 `pending` 状态（还没真发出去），自动改为 `suppressed`，只发 filtered 这一条；
- 如果 `all` 行已经 `sent`（已经飞出去了），不会撤回，但 filtered 仍然正常推送。
