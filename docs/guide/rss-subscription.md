...
> [!WARNING]
> **默认只下载免费种子**：在未启用任何过滤规则的情况下，RSS 订阅**只会自动下载免费种子**，不会下载非免费种子。如果您只是刷流，无需设置任何过滤规则，直接添加 RSS 订阅即可。只有在需要追剧或下载特定资源（即便种子非免费也要下载）时，才需要前往「过滤规则」页面创建规则，并关闭规则中的「仅免费」开关。详见 [过滤规则与追剧指南](filter-rules-tv-series.md)。
