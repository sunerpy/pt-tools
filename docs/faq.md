...
**现象**：免费种子被下载后很快就被 FreeEndMonitor 暂停，进度很低（如 1-2%）。

**原因**：种子的免费剩余时间太短，来不及下载完就到期了。

**解决方案**：

在「系统设置」→「下载策略与限制」中设置「最短免费时间」（默认 30 分钟）。免费剩余时间少于此值的种子会被自动跳过，不再下载。这可以避免浪费下载配额和带宽。

同时建议启用「下载限速判断」并填写实际下载速度，pt-tools 会根据种子大小和免费剩余时间自动判断是否来得及下载完成。
