...
**预期**：退出码 0；`sqlite3 $PT_QA_DB "SELECT COUNT(*) FROM notification_confs"` 输出 = 1

### 2. `inject-tg-cmd.sh`

**用途**：注入 Telegram 入站消息
**用法**：`./inject-tg-cmd.sh "/help"` 或 `./inject-tg-cmd.sh "/torrents qb1" 67890`（可选 user_id）
**预期**：HTTP 200；stdout 为响应 JSON（使用 `jq` 解析 `.ok == true`）
...
**预期**：回复正文匹配 `/(help|status|torrents|search|push|pause|resume|delete|bind|unbind|mute)/g` ≥ 11 次（grep -E 计数）

### 5. `qq-onebot-status.sh`

**用途**：QQ `/status` 回复校验
**预期**：回复同时含 `running` 和 `version` 字样（grep 全文）

### 6. `webhook-event.sh`

**用途**：启动 httptest receiver，触发 `EvtTorrentAdded`，校验 webhook POST 200
**预期**：receiver 收到 1 条 POST，body JSON 含 `event_type=torrent_added`（jq 校验字段）
...
3. `sqlite3` SELECT `channel_binding` 行 +1，`allowed=1` `pt_admin=1`
   **预期**：`channel_bindings` 表行数增 1；新行 `allowed=1` 且 `pt_admin=1`（sqlite3 SELECT 校验）
...
2. 恢复凭证 → 等 outbox worker 1 cycle → `status=sent`
   **预期**：outbox 行 `status` 字段从 `pending` → `sent`（sqlite3 SELECT 跨两次轮询校验）
...
5. 重连后会话清空 → 二次 `/torrents` 不复用旧分页
   **预期**：5 个场景函数全部退出码 0；任一断言失败即整脚本退出码非零
